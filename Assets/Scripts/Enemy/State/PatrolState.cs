    ...
    public class PatrolState : IEnemyState
    {
        ...
        public void UpdateState(EnemyBase enemy)
        {
            ...
            if (enemy.navMeshAgent.pathPending ||
                !(enemy.navMeshAgent.remainingDistance <= enemy.navMeshAgent.stoppingDistance)) return;
            _isWaiting = true;
            ...
            if (_patrolPoint == Vector3.zero) return;
            EnemyPatrolHandler.ReleasePatrolPoint(_patrolPoint);
        ...
        private bool IsObjectInFront(EnemyBase enemy)
        {
            ...
            Vector3 startPosition = new Vector3(position.x, position.y + 1.5f, position.z);
            Vector3 direction = transform.rotation * Vector3.forward; 
        
            return Physics.Raycast(startPosition, direction, out _, 6f);
